...
import EventGalleryPicker from "@/components/Admin/EventGalleryPicker";
...
export default function NewEventPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const ev = await createEvent(token, {
        title, content, location, meetLink, status: eventStatus, imageIds,
      });
      if (imageFile) {
        setStep("uploading");
      ...
      toast.success(eventStatus === "PUBLISHED" ? "Event published!" : "Draft saved!");
  ...
  const btnLabel = step === "creating" ? "Creating…" : step === "uploading" ? "Uploading image…"
    : eventStatus === "PUBLISHED" ? "Publish Event" : "Save Draft";
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Create a new event</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-5">
          <div className="bg-white dark:bg-[#1e2436] rounded-xl border border-gray-100 dark:border-gray-800 p-6 space-y-5">
            ...
            <div>
              ...
              <RichTextEditor value={content} onChange={setContent} placeholder="Describe the event…" minHeight={300} />
            </div>
          </div>
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white dark:bg-[#1e2436] rounded-xl border border-gray-100 dark:border-gray-800 p-5 space-y-4">
            ...
            <button type="submit" disabled={submitting}
              ...
              {submitting ? <><span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />{btnLabel}</> : btnLabel}
            </button>
          </div>
