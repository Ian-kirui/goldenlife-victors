...
export async function generateMetadata({ params }: any) {
  ...
  if (post) {
    return {
      ...
      author: authorName,
      robots: { index: true, follow: true, nocache: true, googleBot: { index: true, follow: false, "max-image-preview": "large", "max-snippet": -1 } },
    };
  }
        ...
        <div className="container lg:max-w-(--breakpoint-xl) md:max-w-(--breakpoint-md) mx-auto">
          <div className="grid md:grid-cols-12 grid-cols-1 items-center">
            ...
            <div className="flex items-center md:justify-center justify-start gap-6 col-span-4 pt-4 md:pt-0">
              ...
              <div>
                <span className="text-[22px] leading-tight font-bold text-midnight_text dark:text-white">{authorName}</span>
            ...
            <div className="w-full px-4">
              {coverImage && (
                <div className="z-20 mb-16 h-80 overflow-hidden rounded-sm md:h-45">
                  <Image src={coverImage} alt={post.title} width={1170} height={766} quality={100} className="h-full w-full object-cover object-center rounded-3xl" />
                </div>
              )}
                ...
                <div className="w-full px-4 lg:w-8/12">
                  ...
                  {post.tags && post.tags.length > 0 && (
                    <div className="mt-8 flex flex-wrap gap-2">
                      {post.tags.map((tag) => (
                        <span key={tag.id} className="text-sm bg-gray-100 dark:bg-dark_border text-gray-600 dark:text-gray-300 px-3 py-1 rounded-full">
                          #{tag.name}
                        </span>
                      ))}
                    </div>
                ...
                <div className="w-full px-4 lg:w-4/12">
                  <div className="-mx-4 mb-8 flex flex-col">
                    <div className="w-full py-12 px-11 bg-white dark:bg-dark shadow-lg border-b-2 border-lightborder dark:border-dark_border rounded-t-lg">
                      <h2 className="wow fadeInUp relative mb-5 text-2xl dark:text-white text-black sm:text-3xl">Share</h2>
                    ...
                    <div className="w-full py-12 px-11 bg-white dark:bg-dark shadow-lg rounded-b-lg">
                      ...
                      <input placeholder="Email address" className="p-3 dark:bg-dark border border-border dark:border-dark_border rounded-lg mb-2 w-full focus:outline-0 focus:border-primary dark:focus:border-primary" />
