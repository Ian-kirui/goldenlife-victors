...
export interface CreateEventPayload {
  ...
  imageIds?: string[];  // IDs from uploadEventGalleryImage
}
...
// ─── Events: Gallery image upload ────────────────────────────────────────────
// POST /events/admin/images/upload — upload one image to the repository,
// returns EventImageResponse { id, url, originalFilename }.
// The returned id is then passed as imageIds[] when creating/updating an event.
export async function uploadEventGalleryImage(
  ...
  const formData = new FormData();
  formData.append("image", imageFile);
  const res = await fetch(`${BASE}/events/admin/images/upload`, {
    method: "POST",
    headers: { Authorization: `Bearer ${token}` },
    body: formData,
    cache: "no-store",
  ...
  if (!res.ok) {
    const body = await res.json().catch(() => null);
    throw new Error(body?.message ?? `Gallery upload failed: ${res.status}`);
  }
  const raw = await res.json();
  // Response: { status, message, data: EventImageResponse }
  return raw.data ?? raw;
}
