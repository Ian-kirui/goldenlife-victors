...
const PARTNER_FORM = "https://forms.gle/your-form-id";
const PAYBILL = "247247";
const ACCOUNT = "071218";

type Tab = "mpesa" | "inkind" | "partner";
type Role = "volunteer" | "partner";
...
export default function SupportForm() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    window.open(PARTNER_FORM, "_blank");
  ...
  return (
    <div className="w-full">
        ...
        {tabs.map((t) => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`flex-1 py-2.5 text-sm font-medium transition-colors ${
              tab === t.key ? "bg-primary text-white" : "text-muted dark:text-white/60 hover:text-primary"
            }`}>
            {t.label}
          </button>
        ))}
      ...
      {tab === "mpesa" && (
        <div className="space-y-4">
          <p className="text-sm text-muted dark:text-white/60 leading-relaxed">
            Send your contribution via M-Pesa Paybill. Please call us after sending to confirm your transaction.
          </p>
          ...
          <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-4">
            <p className="text-sm text-amber-700 dark:text-amber-400">
              📞 After sending, please call us to confirm and for us to acknowledge your support.
            </p>
          </div>
      ...
      {tab === "inkind" && (
        <div className="space-y-4">
          <p className="text-sm text-muted dark:text-white/60 leading-relaxed">
            We welcome physical contributions that directly support our rehabilitation programmes and community outreach activities.
          </p>
          ...
          <p className="text-sm text-muted dark:text-white/60">
            If you are a professional looking to contribute your expertise, please use the <button onClick={() => setTab("partner")} className="text-primary underline">Partner / Volunteer</button> tab.
          </p>
          <a href="tel:+254724571997"
            className="block w-full text-center bg-primary hover:bg-darkprimary text-white font-semibold py-3 rounded-lg transition-colors">
            Call to Arrange: +254 724 571 997
          </a>
        </div>
      )}
      ...
      {tab === "partner" && (
        <form onSubmit={handleSubmit} className="space-y-4">
          {/* Role selector */}
          <div className="flex rounded-lg overflow-hidden border border-border dark:border-dark_border">
            {(["volunteer", "partner"] as Role[]).map((r) => (
              <button key={r} type="button" onClick={() => setRole(r)}
                className={`flex-1 py-2 text-sm font-medium transition-colors capitalize ${
                  role === r ? "bg-primary/10 text-primary border-b-2 border-primary" : "text-muted dark:text-white/60 hover:text-primary"
                }`}>
                {r === "volunteer" ? "Volunteer" : "Partner (Professional)"}
              </button>
            ))}
          </div>
          ...
          <div className="bg-gray-50 dark:bg-gray-800/40 rounded-lg p-3">
            {role === "volunteer" ? (
              <p className="text-xs text-muted dark:text-white/60">
                <strong className="text-midnight_text dark:text-white">Volunteer:</strong> Support our events, outreach activities, and day-to-day operational tasks. No specialist qualifications required — just a willing heart and commitment to showing up.
              </p>
            ) : (
              <p className="text-xs text-muted dark:text-white/60">
                <strong className="text-midnight_text dark:text-white">Partner (Professional):</strong> Apply your professional expertise — clinical, counselling, legal, training, or other specialist skills — to work directly with allocated patients or programme participants under GoldenLife Victors' supervision.
              </p>
            )}
          </div>
          ...
          {role === "partner" && (
            <input required value={form.vocation} onChange={set("vocation")} placeholder="Vocation / Area of expertise *" className={inputCls} />
          )}

          <input value={form.organisation} onChange={set("organisation")} placeholder="Organisation (optional)" className={inputCls} />

          <div>
            <label className="text-xs text-muted dark:text-white/60 mb-1 block">
              {role === "volunteer" ? "Availability duration *" : "Commitment duration *"}
            </label>
            <select required value={form.duration} onChange={set("duration")}
              ...
              <option value="1year">1 Year (Maximum)</option>
            </select>
          </div>
